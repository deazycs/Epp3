...
import { WORKFLOW_STEPS_TEMPLATE } from '@/mock/data/procurements';
...
export default function NovayaZakupkaPage() {
  ...
  const handleSubmit = () => {
    ...
    const newProc: Procurement = {
      ...
      priority,
      isOverdue: false,
      workflowSteps: WORKFLOW_STEPS_TEMPLATE.map((s, i) => ({
        ...s,
        id: `ws-${id}-${i}`,
        procurementId: id,
        isCompleted: false,
            ...
            <div className="gov-card">
              ...
              <div className="p-4 space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <div>
                    <label className="gov-label">Способ закупки / площадка</label>
                    <select className="gov-select" value={method} onChange={e=>setMethod(e.target.value)}>
                      <option>ЕАТ «Берёзка»</option>
                      <option>ЕИС — запрос котировок</option>
                      <option>ЕИС — электронный аукцион</option>
                      <option>ЕИС — конкурс</option>
                      <option>Единственный поставщик (ст.93)</option>
                    </select>
                  </div>
                </div>
              </div>
            ...
            <div className="gov-card p-3">
              ...
              <div className="space-y-1.5">
                {[
                  ...
                  {label:'Тип и приоритет', done:true},
                  {label:'Подразделение', done:!!dept},
                  {label:'Позиции добавлены', done:items.some(i=>!!i.name.trim())},
                  {label:'Суммы указаны', done:totalSum>0},
                  {label:'Сроки установлены', done:!!startDate&&!!endDate},
                  {label:'КБК указан', done:!!kbk.trim()},
                ].map(c=>(
                  <div key={c.label} className="flex items-center gap-2">
                    <span className={`w-4 h-4 rounded-full flex items-center justify-center text-xs flex-shrink-0 ${c.done?'bg-green-500 text-white':'bg-gray-200 text-gray-400'}`}>
                      {c.done?'✓':'○'}
                    </span>
                    <span className={`text-xs ${c.done?'text-green-700 font-bold':'text-gray-500'}`}>{c.label}</span>
                  </div>
                ))}
              </div>
            </div>
